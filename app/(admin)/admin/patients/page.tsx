...
type Patient = { id: string; name: string; email: string; access_type: string; created_at: string };
export default function AdminPatientsPage() {
  ...
  const fetchPatients = async () => {
    setLoading(true);
    const { data } = await supabase.from("patients").select("id, name, email, access_type, created_at").eq("role", "patient").order("created_at", { ascending: false });
  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-2xl font-bold" style={{ color: "var(--foreground)" }}>Patients</h1><p className="text-sm mt-1" style={{ color: "var(--foreground-secondary)" }}>Create and manage patient accounts</p></div>
      ...
      {formSuccess && <div className="mb-6 px-4 py-3 rounded-xl text-sm" style={{ background: "#e8f5e9", color: "#2e7d32" }}>{formSuccess}</div>}
      {showForm && (
        ...
        <div className="card overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="border-b" style={{ borderColor: "var(--border)", background: "var(--card-secondary)" }}>
              {["Name","Email","Access","Joined"].map((h) => <th key={h} className="text-left px-5 py-3 font-medium" style={{ color: "var(--foreground-secondary)" }}>{h}</th>)}
            </tr></thead>

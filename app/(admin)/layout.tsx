      ...
      <aside className="w-64 flex-shrink-0 flex flex-col sidebar-surface" style={{ minHeight: "100vh" }}>
        <div className="p-6 border-b" style={{ borderColor: "var(--border)" }}>
          <Link href="/admin" className="flex items-center gap-2">
            ...
            <div><p className="font-semibold text-sm" style={{ color: "var(--foreground)" }}>Learning Hub</p><p className="text-xs" style={{ color: "var(--foreground-muted)" }}>Admin</p></div>
          </Link>
        </div>
        ...
        <div className="p-4 border-t" style={{ borderColor: "var(--border)" }}>
          <div className="flex items-center gap-3 mb-3">
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: "var(--primary)" }}>
              {(patient?.name || patient?.email || "A")[0].toUpperCase()}
            </div>
            <div className="min-w-0">
              <p className="text-sm font-medium truncate" style={{ color: "var(--foreground)" }}>{patient?.name || "Admin"}</p>
